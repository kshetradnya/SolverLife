...
OUT_DIR = ROOT / "assets/question_snippets"
...
def question_locations(pages):
    # candidates for left-margin question numbers
    ...
    for p_idx, p in enumerate(pages, start=1):
        for w in p["words"]:
            ...
            if w["xMin"] > 75:
                continue
            if w["yMin"] < 40 or w["yMin"] > p["height"] - 55:
                continue
...
def build():
    ...
    for rel_paper, abs_paper in papers.items():
        if not abs_paper.exists():
            continue
        xml_text = sh(["pdftotext", "-bbox-layout", str(abs_paper), "-"])
        pages = parse_bbox_pages(xml_text)
        if not pages:
            continue
        qloc = question_locations(pages)

        # questions from this paper in increasing number
        qnums = sorted({int(q.get("number")) for q in questions if q.get("paper") == rel_paper and q.get("number")})
        for i, n in enumerate(qnums):
            ...
            # end at next question on same page, otherwise to near page end
            y_end = p["height"] - 35
            if i + 1 < len(qnums):
                nxt = qnums[i + 1]
                if nxt in qloc:
            ...
            paper_slug = slug(Path(rel_paper).stem)
            out_png = OUT_DIR / slug(Path(rel_paper).parts[1] if len(Path(rel_paper).parts) > 1 else "paper") / f"{paper_slug}_q{n:02d}.png"
            try:
                crop_question(abs_paper, p_no, y_start - 8, y_end, out_png, p["width"])
                lookup[f"{rel_paper}::{n}"] = str(out_png.relative_to(ROOT)).replace("\\", "/")
                generated += 1
    ...
    (DATA / "question-image-bank.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
    (DATA / "question-image-bank.js").write_text("window.SOLVERLIFE_IMAGE_BANK = " + json.dumps(out, ensure_ascii=True, indent=2) + ";\n", encoding="utf-8")
    print(f"Generated {generated} question snippets")
